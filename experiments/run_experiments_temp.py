...
def setup_confidence_scorer(args, internal_metric, cross_modal_metric, device: str, logger: logging.Logger):
    ...
    if args.use_nli:
        ...
        nli_metric = NLICoherenceMetric(model_name=args.nli_model_name, device=nli_device)
    ...
    if path.is_dir():
        ...
        for sample in samples:
            ...
            for chain_emb in sample_embeddings_data:
                chain_dict = {
                    k: (torch.tensor(v).to(device) if isinstance(v, list) and k not in text_fields else v)
                    for k, v in chain_emb.items()
                }
...
def main():
    ...
    parser.add_argument("--data_path", type=str, default="uno-bench", help="Path to UNO-Bench dataset")
    ...
    parser.add_argument("--cot_model_name", type=str, default="Qwen/Qwen2-Audio-7B-Instruct")
    ...
    parser.add_argument("--text_encoder", type=str, default="sentence-transformers/all-mpnet-base-v2")
    ...
    parser.add_argument("--similarity_metric", type=str, default="cosine", choices=["cosine", "euclidean", "dot"])
    parser.add_argument("--aggregation", type=str, default="mean", choices=["mean", "min", "max"])
    ...
    parser.add_argument("--density_model_type", type=str, default="kde", choices=["kde", "gmm"])
    
    # --- NEW: NLI AND PRM ARGUMENTS ---
    parser.add_argument("--use_nli", action="store_true", help="Enable NLI metric scoring")
    parser.add_argument("--nli_model_name", type=str, default="cross-encoder/nli-deberta-v3-large")
    parser.add_argument("--use_prm", action="store_true", help="Enable PRM metric scoring")
    ...
    # Load dataset
    unobench = UNOBenchLoader(data_path=args.data_path, split=args.split, modality_filter=args.modality_filter)
    ...
    if not args.skip_cot_generation:
        ...
        if args.use_openai_api:
            generator = OpenAICoTGenerator(model_name=args.cot_model_name, api_key=args.openai_api_key)
        else:
            generator = CoTGenerator(model_name=args.cot_model_name, model_type=args.cot_model_type)

        cots = []
        for idx, sample in enumerate(samples):
            try:
                chains = generator.generate_cot_from_sample(sample, max_new_tokens=args.max_new_tokens, num_chains=args.num_chains)
            except Exception as e:
                if args.verbose: print(f"Could not process sample at index {idx}.\nException: {e}")
                continue
            cots.append(chains)
    elif args.load_cots:
    ...
    if args.save_cots:
        save_results(cots=cots, save_cots=args.save_cots, samples=samples, logger=logger)
    ...
    confidence_scorer = setup_confidence_scorer(args, internal_metric, cross_modal_metric, device, logger)
    ...
    if args.sequential_processing:
        ...
        if not args.skip_embedding_extraction:
            text_encoder, multimodal_encoder, audio_encoder = setup_encoders(args, device, logger)
            scores = []
            save_embeddings_dir = Path(args.save_embeddings) if args.save_embeddings else None
            ...
            if not embeddings_path.is_dir():
                logger.error("Sequential processing requires a directory for --load_embeddings")
                return
            ...
            for idx, sample in enumerate(samples):
                embedding_file = embeddings_path / f"{sample.id}.json"
                if not embedding_file.exists(): continue
        ...
        if not args.skip_embedding_extraction:
            text_encoder, multimodal_encoder, audio_encoder = setup_encoders(args, device, logger)
            embeddings = extract_embeddings(samples, cots, text_encoder, multimodal_encoder, audio_encoder, device, logger)
        elif args.load_embeddings:
            embeddings = load_embeddings_from_path(args.load_embeddings, samples, device, logger)
            if args.num_chains_for_scoring is not None:
