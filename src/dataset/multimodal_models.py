...
# ============================================================================
# Unified Interface for CoT Generator Integration
# ============================================================================

# Merge both model maps for unified access
AUDIO_MODELS = {
    "audioflamingo3", "gemma3n", "granite_speech", "midashenglm",
    "minicpmo", "phi4_mm", "phi4_multimodal", "qwen2_audio",
    "qwen2_5_omni", "ultravox", "voxtral", "whisper"
}
...
def format_multimodal_prompt(
    ...
    """
    Unified interface to format multimodal prompts for different models.

    This function serves as a bridge between the UNO-Bench loader, CoT generator,
    and model-specific formatting functions.

    Args:
        model_type: Model identifier (e.g., "llava", "qwen2_audio", etc.)
    ...
    Returns:
        dict: Formatted prompt data ready for VLLM generation with keys:
            - prompt: The formatted prompt string or dict
            - multi_modal_data: Dictionary with image/audio data (if applicable)
            - engine_args: VLLM EngineArgs for model configuration
    ...
    if modality == "auto":
        if images and audio_paths:
            ...
            if model_type in ["qwen2_5_omni", "gemma3n"]:
                ...
                raise ValueError(
                    f"Model {model_type} doesn't support omni-modal inputs. "
                    "Please specify either images or audio, not both."
                )
        elif images:
            modality = "image"
        elif audio_paths:
    ...
    if model_type in AUDIO_MODEL_MAP:
        ...
        req_data = AUDIO_MODEL_MAP[model_type](question, audio_count)
    ...
    elif model_type in VISION_MODEL_MAP:
        # Vision-language model
        questions_list = [question]  # Vision models expect a list

        # Get model-specific formatting
        req_data = VISION_MODEL_MAP[model_type](questions_list, modality)
        ...
        available_models = sorted(list(AUDIO_MODEL_MAP.keys()) + list(VISION_MODEL_MAP.keys()))
        raise ValueError(
            f"Model type '{model_type}' is not supported.\n"
            f"Available models: {', '.join(available_models[:10])}... (and {len(available_models)-10} more)"
        )
